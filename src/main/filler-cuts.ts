...
export function buildKeepSegments(
  ...
  const overlapping = fillerSegments.filter((seg) => seg.start < clipEnd && seg.end > clipStart);
 ...
 */
export function buildSelectFilter(keepSegments: KeepSegment[]): {
  videoSelect: string;
  audioSelect: string;
} | null {
...
export function remapWordTimestamps(
  ...
  const clippedWords = words.filter((w) => w.start >= clipStart && w.end <= clipEnd);
