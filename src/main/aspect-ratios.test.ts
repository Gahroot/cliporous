...
describe('aspect-ratios locked exports', () => {
  ...
  it('locks output dimensions to 720×1280', () => {
    expect(OUTPUT_WIDTH).toBe(720)
    expect(OUTPUT_HEIGHT).toBe(1280)
  ...
  it('getCanvasDimensions always returns 720×1280', () => {
    expect(getCanvasDimensions()).toEqual({ width: 720, height: 1280 })
    expect(getCanvasDimensions('9:16')).toEqual({ width: 720, height: 1280 })
  })
})
...
describe('aspect-ratios resolver — render config output', () => {
  ...
  for (const src of sources) {
    it(`resolves ${src.label} (${src.width}×${src.height}) to 720×1280 @ 30fps`, () => {
      const config = resolveRenderConfig(src.width, src.height)

      expect(config.output.width).toBe(720)
  ...
  it('every source resolves to identical output dimensions and fps', () => {
    ...
    for (const out of outputs) {
      expect(out).toEqual({ width: 720, height: 1280, fps: 30 })
    }
  })
})
